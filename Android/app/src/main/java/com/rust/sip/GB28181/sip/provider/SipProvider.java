...
package com.rust.sip.GB28181.sip.provider;
...
import com.rust.sip.GB28181.net.SocketAddress;
import com.rust.sip.GB28181.net.TcpServer;
import com.rust.sip.GB28181.net.TcpServerListener;
import com.rust.sip.GB28181.net.TcpSocket;
...
import com.rust.sip.GB28181.tools.Configurable;
import com.rust.sip.GB28181.tools.Configure;
import com.rust.sip.GB28181.tools.DateFormat;
import com.rust.sip.GB28181.tools.HashSet;
import com.rust.sip.GB28181.tools.Iterator;
import com.rust.sip.GB28181.tools.Log;
import com.rust.sip.GB28181.tools.LogLevel;
import com.rust.sip.GB28181.tools.Logger;
import com.rust.sip.GB28181.tools.Parser;
import com.rust.sip.GB28181.tools.Random;
import com.rust.sip.GB28181.tools.RotatingLog;
import com.rust.sip.GB28181.tools.SimpleDigest;
...
public class SipProvider implements Configurable, TransportListener,
	 ...
	 * 是否添加rport 参数于 outgoing requests*/
	boolean rport = true;
	...
	public void parseLine(String line) {
		...
		if (attribute.equals("host_addr"))
			System.err
					.println("WARNING: parameter 'host_addr' is no more supported; use 'via_addr' instead.");
...
//			wl = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "Rokid:Sipdroid.SipProvider");
//		}
//		wl.acquire(); // modified
	...
	public void onTransportTerminated(Transport transport, Exception error) {
		...
		if (transport.getProtocol().equals(PROTO_TCP)) {
			...
			if (SipEngine.on(Receiver.mContext))
				Receiver.engine(Receiver.mContext).register(); // modified
		}
		if (error != null)
	...
	private SipURL completeSipURL(String str) { // in case it is passed only the
		...
		if (!str.startsWith("sip:") && str.indexOf("@") < 0
				&& str.indexOf(".") < 0 && str.indexOf(":") < 0) { // may be it
			// is just
			// the user
			// name..
			String url = "sip:" + str + "@";
