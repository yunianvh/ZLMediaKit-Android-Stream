...
import com.rust.sip.GB28181.sip.dialog.Dialog;
import com.rust.sip.GB28181.sip.header.ContactHeader;
import com.rust.sip.GB28181.sip.header.EventHeader;
import com.rust.sip.GB28181.sip.header.ReferToHeader;
import com.rust.sip.GB28181.sip.header.ReferredByHeader;
...
public class MessageFactory extends BaseMessageFactory {
	...
	public static Message createMessageRequest(SipProvider sip_provider,
											   NameAddress recipient, NameAddress from, String subject,
	...
	public static Message createInviteRequest(SipProvider sip_provider,
											   NameAddress recipient,NameAddress to, NameAddress from, String subject, NameAddress contact,long cseq,
											   String type, String body) {
		SipURL request_uri = recipient.getAddress();
		String callid = sip_provider.pickCallId();
		String localtag = SipProvider.pickTag();
		// String branch=SipStack.pickBranch();
		Message req = createRequest(sip_provider, SipMethods.dialog_methods[0],
				request_uri, recipient, from, null, callid, cseq, localtag,
				null, null, null, null);									// modified by mandrajg
		...
		if (subject != null)
			req.setSubjectHeader(new SubjectHeader(subject));
		if (contact != null)
			req.setContactHeader(new ContactHeader(contact));
		req.setBody(type, body);
		return req;
	}
	/** Creates a new REFER request (RFC3515) */
	public static Message createReferRequest(SipProvider sip_provider,
			NameAddress recipient, NameAddress from, NameAddress contact,
			NameAddress refer_to/* , NameAddress referred_by */) {
		SipURL request_uri = recipient.getAddress();
		String callid = sip_provider.pickCallId();
		int cseq = SipProvider.pickInitialCSeq();
		String localtag = SipProvider.pickTag();
	 ...
	 * <p>
	 * parameters: <br> - <i>refer_to</i> mandatory <br> - <i>referred_by</i>
	 * optional
	 */
	public static Message createReferRequest(Dialog dialog,
		...
		if (referred_by != null)
			...
			req
					.setReferredByHeader(new ReferredByHeader(dialog
							.getLocalName()));
		return req;
	}
	 ...
	 * dialogs.
	 */
	public static Message createSubscribeRequest(SipProvider sip_provider,
			SipURL recipient, NameAddress to, NameAddress from,
			NameAddress contact, String event, String id, String content_type,
			String body) {
		Message req = createRequest(sip_provider, SipMethods.SUBSCRIBE,
	...
	public static Message createSubscribeRequest(Dialog dialog, String event,
			String id, String content_type, String body) {
