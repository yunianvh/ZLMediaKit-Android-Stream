...
import com.rust.sip.GB28181.net.KeepAliveSip;
...
public class SipdroidEngine implements RegisterAgentListener, UserAgent.OnRingCallback {
	...
	public boolean StartEngine() {
			...
			for (UserAgentProfile user_profile : user_profiles) {
				if (wl[i] == null) {
					wl[i] = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "Rokid:Sipdroid.SipdroidEngine");
					pwl[i] = pm.newWakeLock(PowerManager.SCREEN_DIM_WAKE_LOCK | PowerManager.ACQUIRE_CAUSES_WAKEUP, "Rokid:Sipdroid.SipdroidEngine");
		...
		//			SipStack.log_path = "/data/data/org.sipdroid.sipua";
					...
					kas[i] = new KeepAliveSip(sip_providers[i],user_profile.from_url,user_profile.username);
				} catch (Exception E) {
				}
				i++;
			}
...
//	 * 获取audio info
//	 */
//	public JAudioLauncher getAudio() {
//		return (JAudioLauncher) ua.audio_app;
//	}
...
//	}
	...
	int getKeepaliveInterval(int i) {
			if (Build.VERSION.SDK_INT >= 24)
				return 0;
			if (sip_providers[i].getDefaultTransport().equals("UDP")||sip_providers[i].getDefaultTransport().equals("udp")) {
				return 20;
			} else
				return 5 * 60;
	}
    ...
    @Override
    public void onUaRegistrationGb28181Success(NameAddress target) {
        for(KeepAliveSip ka:kas){
            ka.setToTaget(target.getAddress().toString());
        }
    }
...
//					if (!Sipdroid.release)
    ...
    @Override
	public void onCallRinging(UserAgent agent) {
		Log.e("ringCallback", "onCallRinging");
	}

	@Override
	public void onCallConfirmed(UserAgent agent,UserAgent.onRingParam param) {
		if(sipdroidEngineCallback!=null){
			sipdroidEngineCallback.onCallAccepted(agent,param);
		}
		Log.e("ringCallback", "onCallRinging");
	...
	@Override
	public void onCallClosing(UserAgent agent) {
		if(sipdroidEngineCallback!=null){
	...
	public interface SipdroidEngineCallback{
		void onCallAccepted(UserAgent agent, UserAgent.onRingParam param);
		void onCallClosing();
	}
}
